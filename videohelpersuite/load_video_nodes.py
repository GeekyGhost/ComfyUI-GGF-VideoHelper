...
"""
External plugins may add additional formats to nodes.GGFLoadFormats
...
"""
if not hasattr(nodes, 'GGFLoadFormats'):
    nodes.GGFLoadFormats = {}

def get_load_formats():
    #TODO: check if {**extra_config.GGFLoadFormats, **GGFLoadFormats} has minimum version
    formats = {}
    formats.update(nodes.GGFLoadFormats)
    formats.update(GGFLoadFormats)
    return (list(formats.keys()),
            {'default': 'AnimateDiff', 'formats': formats})
def get_format(format):
    if format in GGFLoadFormats:
        return GGFLoadFormats[format]
    return nodes.GGFLoadFormats.get(format, {})
...
class LoadVideoUpload:
    ...
    RETURN_TYPES = (imageOrLatent, "INT", "AUDIO", "GGF_VIDEOINFO")
...
class LoadVideoPath:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "video": ("STRING", {"placeholder": "X://insert/path/here.mp4", "ggf_path_extensions": video_extensions}),
                "force_rate": (floatOrInt, {"default": 0, "min": 0, "max": 60, "step": 1, "disable": 0}),
                "custom_width": ("INT", {"default": 0, "min": 0, "max": DIMMAX, 'disable': 0}),
                "custom_height": ("INT", {"default": 0, "min": 0, "max": DIMMAX, 'disable': 0}),
                "frame_load_cap": ("INT", {"default": 0, "min": 0, "max": BIGMAX, "step": 1, "disable": 0}),
...
class LoadVideoFFmpegUpload:
    ...
    def INPUT_TYPES(s):
        ...
        return {"required": {
                    ...
                    "custom_height": ("INT", {"default": 0, "min": 0, "max": DIMMAX, 'disable': 0}),
                    "frame_load_cap": ("INT", {"default": 0, "min": 0, "max": BIGMAX, "step": 1, "disable": 0}),
                    "start_time": ("FLOAT", {"default": 0, "min": 0, "max": BIGMAX, "step": .001, "widgetType": "GGFTIMESTAMP"}),
                    },
                "optional": {
                    "meta_batch": ("GGF_BatchManager",),
                    "vae": ("VAE",),
    ...
    RETURN_TYPES = (imageOrLatent, "MASK", "AUDIO", "GGF_VIDEOINFO")
...
class LoadImagePath:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("STRING", {"placeholder": "X://insert/path/here.png", "ggf_path_extensions": list(FolderOfImages.IMG_EXTENSIONS)}),
