...
if "GGF_FORCE_FFMPEG_PATH" in os.environ:
    ffmpeg_path = os.environ.get("GGF_FORCE_FFMPEG_PATH")
else:
    ffmpeg_paths = []
    try:
        ...
        if "GGF_USE_IMAGEIO_FFMPEG" in os.environ:
            raise
...
gifski_path = os.environ.get("GGF_GIFSKI", None)
if gifski_path is None:
    gifski_path = os.environ.get("JOV_GIFSKI", None)
    if gifski_path is None:
        gifski_path = shutil.which("gifski")
ytdl_path = os.environ.get("GGF_YTDL", None) or shutil.which('yt-dlp') \
...
def is_safe_path(path, strict=False):
    if "GGF_STRICT_PATHS" not in os.environ and not strict:
        return True
...
def requeue_workflow_unchecked():
    ...
    prompt = prompt.copy()
    for uid in prompt:
        if prompt[uid]['class_type'] == 'GGF_BatchManager':
...
def requeue_workflow(requeue_required=(-1,True)):
    ...
    if requeue_guard[0] != run_number:
        ...
        for bm_uid in prompt:
            if prompt[bm_uid]['class_type'] == 'GGF_BatchManager':
                for output_uid in prompt:
                    if prompt[output_uid]['class_type'] in ["GGF_VideoCombine"]:
...
def get_audio(file, start_time=0, duration=0):
    ...
    try:
        ...
        raise Exception(f"GGF failed to extract audio from {file}:\n" \
                + e.stderr.decode(*ENCODE_ARGS))
    if match:
