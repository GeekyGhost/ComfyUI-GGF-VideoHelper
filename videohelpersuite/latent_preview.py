...
class WrappedPreviewer(latent_preview.LatentPreviewer):
    def __init__(self, previewer, rate=8):
        ...
        if hasattr(previewer, 'taesd'):
            ...
            raise Exception('Unsupported preview type for GGF animated previews')
    ...
    def decode_latent_to_preview_image(self, preview_format, x0):
        ...
        if self.first_preview:
            ...
            serv.send_sync('GGF_latentpreview', {'length':num_images, 'rate': self.rate, 'id': serv.last_node_id})
...
def get_latent_video_previewer(device, latent_format, *args, **kwargs):
    ...
    try:
        ...
        prev_setting = extra_info.get('GGF_latentpreview', False)
        if extra_info.get('GGF_latentpreviewrate', 0) != 0:
            rate_setting = extra_info['GGF_latentpreviewrate']
        else:
