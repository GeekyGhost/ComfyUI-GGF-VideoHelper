...
def video(src):
    return f'<video preload="none" src={src} muted loop controls controlslist="nodownload noremoteplayback noplaybackrate" style="width: 0px; min-width: 100%" class="GGF_loopedvideo">'
def short_desc(desc):
    return f'<div id=GGF_shortdesc>{desc}</div>'
...
common_descriptions = {
  ...
  'GGF_PATH': [
    'This is a GGF_PATH input. When edited, it provides a list of possible valid files or directories',
...
descriptions = {
  'GGF_VideoCombine': ['Video Combine 🎥🅖🅖🅕', short_desc('Combine an image sequence into a video'), {
    'Inputs': {
    ...
    'Common Format Widgets': {
        ...
        'input_color_depth': 'GGF supports outputting 16bit images. While this produces higher quality output, the difference usually isn\'t visible without postprocessing and it significantly increases file size and processing time.',
        'save_metadata': 'Determines if metadata for the workflow should be included in the output video file',
        }
    }],
  'GGF_LoadVideo': ['Load Video 🎥🅖🅖🅕', short_desc('Loads a video from the input folder'),
    {'Inputs': {
  ...
  'GGF_LoadVideoFFmpeg': ['Load Video FFmpeg 🎥🅖🅖🅕', short_desc('Loads a video from the input folder using ffmpeg instead of opencv'),
  ...
  'GGF_LoadVideoPath': ['Load Video (Path) 🎥🅖🅖🅕', short_desc('Loads a video from an arbitrary path'),
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        'vae': ['(optional) If provided the node will output latents instead of images. This drastically reduces the required RAM (not VRAM) when working with long (100+ frames) sequences',
                'Using this is strongly encouraged unless connecting to a node that requires a blue image connection such as Apply Controllnet',
     ...
     'Widgets': {
         'video': ['The video file to be loaded.', 'You can also select an image to load it as a single frame'] + common_descriptions['GGF_PATH'],
         'force_rate': 'Drops or duplicates frames so that the produced output has the target frame rate. Many motion models are trained on videos of a specific frame rate and will give better results if input matches that frame rate. If set to 0, all frames are returned. May give unusual results with inputs that have a variable frame rate like animated gifs. Reducing this value can also greatly reduce the execution time and memory requirements.',
         'force_size': 'Previously was used to provide suggested resolutions. Instead, custom_width and custom_height can be disabled by setting to 0.',
         'custom_width': 'Allows for an arbitrary width to be entered, cropping to maintain aspect ratio if both are set',
         'custom_height': 'Allows for an arbitrary height to be entered, cropping to maintain aspect ratio if both are set',
  ...
  'GGF_LoadVideoFFmpegPath': ['Load Video FFmpeg (Path) 🎥🅖🅖🅕', short_desc('Loads a video from an arbitrary path using ffmpeg instead of opencv'),
    'Provides faster execution speed, transparency support, and allows specifying start time in seconds',
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        'vae': ['(optional) If provided the node will output latents instead of images. This drastically reduces the required RAM (not VRAM) when working with long (100+ frames) sequences',
  ...
  'GGF_LoadImages': ['Load Images 🎥🅖🅖🅕', short_desc('Loads a sequence of images from a subdirectory of the input folder'),
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        },
     'Outputs': {
  ...
  'GGF_LoadImagesPath': ['Load Images (Path) 🎥🅖🅖🅕', short_desc('Loads a sequence of images from an arbitrary path'),
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        },
     'Outputs': {
     ...
     'Widgets': {
         'directory': ['The directory images will be loaded from. Filtered to process jpg, png, ppm, bmp, tif, and webp files'] + common_descriptions['GGF_PATH'],
         'image_load_cap': 'The maximum number of images to load. If 0, all images are loaded.',
  ...
  'GGF_LoadImagePath': ['Load Image (Path) 🎥🅖🅖🅕', short_desc('Load a single image from a given path'),
    {'Inputs': {
     ...
     'Widgets': {
         'image': ['The image file to be loaded.'] + common_descriptions['GGF_PATH'],
  ...
  "GGF_LoadAudio": ['Load Audio (Path) 🎥🅖🅖🅕', short_desc('Loads an audio file from an arbitrary path'),
    {'Outputs': {
         'audio': 'The loaded audio',
         },
     'Widgets': {
         'audio_file': ['The audio file to be loaded.'] + common_descriptions['GGF_PATH'],
         'seek_seconds': 'An offset from the start of the sound file that the audio should start from',
         }
        }],
  "GGF_LoadAudioUpload": ['Load Audio (Upload) 🎥🅖🅖🅕', short_desc('Loads an audio file from the input directory'),
    "Very similar in functionality to the built-in LoadAudio. It was originally added before GGF swapped to use Comfy's internal AUDIO format, but provides the additional options for start time and duration",
    {'Outputs': {
         'audio': 'The loaded audio',
         },
     'Widgets': {
  ...
  "GGF_AudioToGGFAudio": ['Audio to GGF_AUDIO 🎥🅖🅖🅕', short_desc('utility function for compatibility with external nodes'),
    "GGF used to use an internal GGF_AUDIO format for routing audio between inputs and outputs. This format was intended to only be used internally and was designed with a focus on performance over ease of use. Since ComfyUI now has an internal AUDIO format, GGF now uses this format. However, some custom node packs were made that are external to both ComfyUI and GGF that use GGF_AUDIO. This node was added so that those external nodes can still function",
    {'Inputs': {
        'audio': 'An input in the standardized AUDIO format',
        },
     'Outputs': {
         'ggf_audio': 'An output in the legacy GGF_AUDIO format for use with external nodes',
         },
        }],
  "GGF_GGFAudioToAudio": ['GGF_AUDIO to Audio 🎥🅖🅖🅕', short_desc('utility function for compatibility with external nodes'),
    "GGF used to use an internal GGF_AUDIO format for routing audio between inputs and outputs. This format was intended to only be used internally and was designed with a focus on performance over ease of use. Since ComfyUI now has an internal AUDIO format, GGF now uses this format. However, some custom node packs were made that are external to both ComfyUI and GGF that use GGF_AUDIO. This node was added so that those external nodes can still function",
    {'Inputs': {
        'ggf_audio': 'An input in the legacy GGF_AUDIO format produced by an external node',
        },
     'Outputs': {
         'ggf_audio': 'An output in the standardized AUDIO format',
         },
        }],
  "GGF_PruneOutputs": ['Prune Outputs 🎥🅖🅖🅕', short_desc('Automates deletion of undesired outputs from a Video Combine node.'),
  ...
  "GGF_BatchManager": ['Meta Batch Manager 🎥🅖🅖🅕', short_desc('Split the processing of a very long video into sets of smaller Meta Batches'),
  ...
  "GGF_VideoInfo": ['Video Info 🎥🅖🅖🅕', short_desc('Splits information on a video into a numerous outputs'),
    {'Inputs': {
  ...
  "GGF_VideoInfoSource": ['Video Info Source 🎥🅖🅖🅕', short_desc('Splits information on a video into a numerous outputs describing the file itself without accounting for load options'),
    {'Inputs': {
        'video_info': 'A connection to a Load Video node',
        },
     'Outputs': {
  ...
  "GGF_VideoInfoLoaded": ['Video Info Loaded 🎥🅖🅖🅕', short_desc('Splits information on a video into a numerous outputs describing the file itself after accounting for load options'),
    {'Inputs': {
        'video_info': 'A connection to a Load Video node',
        },
     'Outputs': {
  ...
  "GGF_SelectFilename": ['VAE Select Filename 🎥🅖🅖🅕', short_desc('Select a single filename from the GGF_FILENAMES output by a Video Combine and return it as a string'),
    'Take care when combining this node with Prune Outputs. The GGF_FILENAMES object is immutable and will always contain the full list of output files, but execution order is undefined behavior (currently, Prune Outputs will generally execute first) and SelectFilename may return a path to a file that no longer exists.',
    {'Inputs': {
        'filenames': 'A GGF_FILENAMES from a Video Combine node',
        },
     'Outputs': {
    ...
    # Batched Nodes
  "GGF_VAEEncodeBatched": ['VAE Encode Batched 🎥🅖🅖🅕', short_desc('Encode images as latents with a manually specified batch size.'),
  ...
  "GGF_VAEDecodeBatched": ['VAE Decode Batched 🎥🅖🅖🅕', short_desc('Decode latents to images with a manually specified batch size'),
  ...
  "GGF_SplitLatents": ['Split Latents 🎥🅖🅖🅕', short_desc('Split a set of latents into two groups'),
    {'Inputs': {
    ...
    "GGF_SplitImages": ['Split Images 🎥🅖🅖🅕', short_desc('Split a set of images into two groups'),
    {'Inputs': {
    ...
    "GGF_SplitMasks": ['Split Masks 🎥🅖🅖🅕', short_desc('Split a set of masks into two groups'),
    {'Inputs': {
    ...
    "GGF_MergeLatents": ['Merge Latents 🎥🅖🅖🅕', short_desc('Combine two groups of latents into a single group of latents'),
    {'Inputs': {
    ...
    "GGF_MergeImages": ['Merge Images 🎥🅖🅖🅕', short_desc('Combine two groups of images into a single group of images'),
    {'Inputs': {
    ...
    "GGF_MergeMasks": ['Merge Masks 🎥🅖🅖🅕', short_desc('Combine two groups of masks into a single group of masks'),
    {'Inputs': {
        'mask_A': 'The first group of masks',
    ...
    "GGF_GetLatentCount": format_type(common_descriptions['GetCount'], 'latent'),
    "GGF_GetImageCount": format_type(common_descriptions['GetCount'], 'image'),
    "GGF_GetMaskCount": format_type(common_descriptions['GetCount'], 'mask'),
    "GGF_DuplicateLatents": ['Repeat Latents 🎥🅖🅖🅕', short_desc('Append copies of a latent to itself so it repeats'),
    {'Inputs': {
    ...
    "GGF_DuplicateImages": ['Repeat Images 🎥🅖🅖🅕', short_desc('Append copies of a image to itself so it repeats'),
    {'Inputs': {
    ...
    "GGF_DuplicateMasks": ['Repeat Masks 🎥🅖🅖🅕', short_desc('Append copies of a mask to itself so it repeats'),
    {'Inputs': {
    ...
    "GGF_SelectEveryNthLatent": format_type(common_descriptions['SelectEveryNth'], 'latent'),
    "GGF_SelectEveryNthImage": format_type(common_descriptions['SelectEveryNth'], 'image'),
    #TODO: fix discrepency of input being mask instead of masks?
    "GGF_SelectEveryNthMask": format_type(common_descriptions['SelectEveryNth'], 'mask', lowers='mask'),
    #TODO: port documentation for select nodes to new system
    #"GGF_SelectLatents": None,
    #"GGF_SelectImages": None,
    #"GGF_SelectMasks": None,
  "GGF_Unbatch": ['Unbatch 🎥🅖🅖🅕', short_desc('Unbatch a list of items into a single concatenated item'),
    "Useful for when you want a single video output from a complex workflow",
    "Has no relation to the Meta Batch system of GGF",
    {'Inputs': {
  ...
  "GGF_SelectLatest": ['Select Latest 🎥🅖🅖🅕', short_desc('Experimental virtual node to select the most recently modified file from a given folder'),
    ...
    if isinstance(entry, dict):
        ...
        for k in entry:
            ...
            if collapse_single:
                ...
                name = k
            collapse_flag = ' GGF_precollapse' if entry.get("collapsed", False) or collapse_single else ''
            html += f'<div vhs_title=\"{name}\" style=\"display: flex; font-size: {size}em\" class=\"GGF_collapse{collapse_flag}\"><div style=\"color: #AAA; height: 1.5em;\">[<span style=\"font-family: monospace\">-</span>]</div><div style=\"width: 100%\">{name}: {as_html(entry[k], depth=depth+1)}</div></div>'
        return html
