...
const convDict = {
    GGF_LoadImages : ["directory", null, "image_load_cap", "skip_first_images", "select_every_nth"],
    GGF_LoadImagesPath : ["directory", "image_load_cap", "skip_first_images", "select_every_nth"],
    GGF_VideoCombine : ["frame_rate", "loop_count", "filename_prefix", "format", "pingpong", "save_image"],
    GGF_LoadVideo : ["video", "force_rate", "force_size", "frame_load_cap", "skip_first_frames", "select_every_nth"],
    GGF_LoadVideoPath : ["video", "force_rate", "force_size", "frame_load_cap", "skip_first_frames", "select_every_nth"],
};
const renameDict  = {GGF_VideoCombine : {save_output : "save_image"}}
...
function initHelpDOM() {
    ...
    parentDOM.className = "GGF_floatinghelp"
    ...
    scrollbarStyle.innerHTML = `
            ...
            .GGF_loopedvideo::-webkit-media-controls-mute-button {
                display:none;
            }
            .GGF_loopedvideo::-webkit-media-controls-fullscreen-button {
                display:none;
            }
    `
    ...
    helpDOM.selectHelp = function(name, value) {
        ...
        function collapseUnlessMatch(items,t) {
            ...
            window.scrollTo(0,0)
            for (let i of items.querySelectorAll('.GGF_collapse')) {
                if (i.contains(match)) {
                    setCollapse(i, false)
                } else {
                    setCollapse(i, true)
        ...
        chainCallback(node, "onMouseDown", function (e, pos, canvas) {
            ...
            if (pos[1] < 0 && pos[0] + LiteGraph.NODE_TITLE_HEIGHT > this.size[0]) {
                ...
                if (helpDOM.node == this) {
                    ...
                    for (let e of helpDOM.querySelectorAll('.GGF_collapse')) {
                        e.children[0].onclick = helpDOM.collapseOnClick
                        e.children[0].style.cursor = 'pointer'
                    }
                    for (let e of helpDOM.querySelectorAll('.GGF_precollapse')) {
                        setCollapse(e, true)
                    }
                    for (let e of helpDOM.querySelectorAll('.GGF_loopedvideo')) {
                        e?.play()
                    }
...
function cloneType(nodeType, nodeData) {
    chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        this.changeOutputType("GGF_DUMMY_NONE")
    });
    chainCallback(nodeType.prototype, "onConnectionsChange", function(contype, slot, iscon, linf) {
        if (contype == LiteGraph.INPUT && slot == 0) {
            let new_type = "GGF_DUMMY_NONE"
            if (iscon && linf) {
...
function addAudioPreview(nodeType, isInput=true) {
    chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        this.updateParameters = (params, force_update) => {
            ...
            if (!force_update &&
                app.ui.settings.getSettingValue("GGF.AdvancedPreviews") == 'Never') {
                return;
            }
            if (timeout) {
                clearTimeout(timeout);
        ...
        previewWidget.updateSource = function () {
            ...
            if (!advp) {
                ...
                params.deadline = app.ui.settings.getSettingValue("GGF.AdvancedPreviewsDeadline")
                element.src = api.apiURL('/ggf_vhs/viewaudio?' + new URLSearchParams(params));
            }
        }
...
function addVideoPreview(nodeType, isInput=true) {
    chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        previewWidget.value = {hidden: false, paused: false, params: {},
            muted: app.ui.settings.getSettingValue("GGF.DefaultMute")}
        previewWidget.parentEl = document.createElement("div");
        previewWidget.parentEl.className = "ggf_preview";
        ...
        previewWidget.updateSource = function () {
            ...
            if (params.format?.split('/')[0] == 'video'
                ...
                if (!advp) {
                    ...
                    let minWidth = app.ui.settings.getSettingValue("GGF.AdvancedPreviewsMinWidth")
                    ...
                    params.deadline = app.ui.settings.getSettingValue("GGF.AdvancedPreviewsDeadline")
                    this.videoEl.src = api.apiURL('/ggf_vhs/viewvideo?' + new URLSearchParams(params));
                }
            ...
            const doQuery = async () => {
                ...
                let qurl = api.apiURL('/ggf_vhs/queryvideo?' + new URLSearchParams(previewWidget.value.params))
                let query = undefined
                try {
...
function addPreviewOptions(nodeType) {
    chainCallback(nodeType.prototype, "getExtraMenuOptions", function(_, options) {
        ...
        optNew.push({content: "Sync preview", callback: () => {
            ...
            for (let p of document.getElementsByClassName("ggf_preview")) {
...
function searchBox(event, [x,y], node) {
    ...
    var timeout = null;
    let last_path = null;
    let extensions = pathWidget.options.ggf_path_extensions
    ...
    async function updateOptions() {
        ...
        if (last_path != path) {
            ...
            let optionsURL = api.apiURL('/ggf_vhs/getpath?' + new URLSearchParams(params));
            try {
            ...
            input.addEventListener("keydown", (e) => {
                if (e.keyCode == 9) {
                    ...
                    if (e.shiftKey)
                        ...
                        i++
                    if (node.widgets[i]?.type == "GGF.ANNOTATED") {//restrict to annotatedNUmbers
...
app.registerExtension({
    name: "GGF.VideoHelperSuite.Core",
    settings: [
      {
        id: 'GGF.AdvancedPreviews',
      ...
      {
        id: 'GGF.AdvancedPreviewsMinWidth',
      ...
      {
        id: 'GGF.AdvancedPreviewsDeadline',
      ...
      {
        id: 'GGF.AdvancedPreviewsDefaultMute',
        ...
        onChange(value) {
            if (!value) {
                ...
                for (let id of latentPreviewNodes) {
                    ...
                    let i = n?.widgets?.findIndex((w) => w.name == 'ggflatentpreview')
                    if (i >= 0) {
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if(nodeData?.name?.startsWith("GGF_")) {
            useKVState(nodeType);
            if (nodeData.description) {
                ...
                if (!el.children.length) {
                    ...
                    nodeData.description = el.querySelector('#GGF_shortdesc')?.innerHTML || el.children[1]?.firstChild?.innerHTML
                }
                chainCallback(nodeType.prototype, "onNodeCreated", function () {
            ...
            chainCallback(nodeType.prototype, "onNodeCreated", function () {
                ...
                if (this.widgets) {
                    for (let w of this.widgets) {
                        ...
                        if (w?.type == "text" && config[1].ggf_path_extensions) {
                            new_widgets.push(app.widgets.GGFPATH({}, w.name, ["GGFPATH", config[1]]));
                        } else {
                            new_widgets.push(w)
                        }
                    }
        ...
        if (nodeData?.name == "GGF_LoadImages") {
        ...
        } else if (nodeData?.name == "GGF_LoadImagesPath") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
                const pathWidget = this.widgets.find((w) => w.name === "directory");
                chainCallback(pathWidget, "callback", (value) => {
                    if (!value) {
            ...
            addLoadCommon(nodeType, nodeData);
        } else if (nodeData?.name == "GGF_LoadVideo" || nodeData?.name == "GGF_LoadVideoFFmpeg") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        } else if (nodeData?.name == "GGF_LoadAudio") {
        ...
        } else if (nodeData?.name == "GGF_LoadAudioUpload") {
        ...
        } else if (nodeData?.name == "GGF_LoadVideoPath" || nodeData?.name == "GGF_LoadVideoFFmpegPath") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
                const pathWidget = this.widgets.find((w) => w.name === "video");
                chainCallback(pathWidget, "callback", (value) => {
            ...
            addLoadCommon(nodeType, nodeData);
            addVAEOutputToggle(nodeType, nodeData);
        } else if (nodeData?.name == "GGF_LoadImagePath") {
            addLoadCommon(nodeType, nodeData);
            addVAEOutputToggle(nodeType, nodeData);
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        } else if (nodeData?.name == "GGF_VideoCombine") {
        ...
        } else if (nodeData?.name == "GGF_SaveImageSequence") {
    ...
    async getCustomWidgets() {
        return {
            GGFPATH(node, inputName, inputData) {
                let w = {
                    name : inputName,
                    type : "GGF.PATH",
                    value : "",
            ...
            GGFFLOAT(node, inputName, inputData) {
                let w = {
                    name: inputName,
                    type: "GGF.ANNOTATED",
            ...
            GGFINT(node, inputName, inputData) {
                let w = {
                    name: inputName,
                    type: "GGF.ANNOTATED",
                    value: inputData[1]?.default ?? 0,
            ...
            GGFTIMESTAMP(node, inputName, inputData) {
                let w = {
                    name: inputName,
                    type: "GGF.TIMESTAMP",
                    value: inputData[1]?.default ?? 0,
    ...
    async loadedGraphNode(node) {
        ...
        if (node.type?.startsWith("GGF_") && node.inputs) {
    ...
    async setup() {
        ...
        let graphToPrompt = async function() {
            ...
            res.workflow.extra['GGF_latentpreview'] = app.ui.settings.getSettingValue("GGF.LatentPreview")
            res.workflow.extra['GGF_latentpreviewrate'] = app.ui.settings.getSettingValue("GGF.LatentPreviewRate")
            res.workflow.extra['GGF_MetadataImage'] = app.ui.settings.getSettingValue("GGF.MetadataImage")
            res.workflow.extra['GGF_KeepIntermediate'] = app.ui.settings.getSettingValue("GGF.KeepIntermediate")
            return res
        }
        ...
        document.addEventListener('paste', async (e) => {
            ...
            if (filepath && copiedPath == filepath) {
                ...
                const pastedNode = LiteGraph.createNode('GGF_LoadVideoPath')
                ...
                const pastedNode = LiteGraph.createNode('GGF_LoadVideo')
                app.canvas.graph.add(pastedNode)
                pastedNode.pos[0] = app.canvas.graph_mouse[0]
                pastedNode.pos[1] = app.canvas.graph_mouse[1]
    ...
    async init() {
        if (app.ui.settings.getSettingValue("GGF.AdvancedPreviews") == true) {
            app.ui.settings.setSettingValue("GGF.AdvancedPreviews", 'Always')
        }
        if (app.ui.settings.getSettingValue("GGF.AdvancedPreviews") == false) {
            app.ui.settings.setSettingValue("GGF.AdvancedPreviews", 'Never')
        }
        if (app.GGFHelp != helpDOM) {
            helpDOM = app.GGFHelp
        } else {
            initHelpDOM()
        }
...
function getLatentPreviewCtx(id, width, height) {
    ...
    let previewWidget = node.widgets.find((w) => w.name == "ggflatentpreview")
    if (!previewWidget) {
        ...
        previewWidget = node.addDOMWidget("ggflatentpreview", "ggfcanvas", canvasEl, {
            serialize: false,
            hideOnZoom: false,
        });
...
api.addEventListener('GGF_latentpreview', ({ detail }) => {
    if (detail.id == null) {
        return
    }
